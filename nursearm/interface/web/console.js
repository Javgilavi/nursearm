...
function addChatMessage(role, text) {
  ...
  title.textContent = role === "user" ? "Patient" : "NurseArm";
...
async function dispenseGreenPill() {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.detail || `Handover skill failed: ${response.status}`);
  return data;
}
...
async function loadRemoteUrl() {
  try {
    ...
    if (!data.ngrok_url) return;

    // Desktop sidebar QR
    const qrSection = document.getElementById("qr-section");
    const qrImg = document.getElementById("qr-image");
    const ngrokLink = document.getElementById("ngrok-link");
    if (qrSection && qrImg && ngrokLink) {
      qrImg.src = "/qr.svg";
      ngrokLink.href = data.ngrok_url;
      ngrokLink.textContent = data.ngrok_url.replace("https://", "");
      qrSection.hidden = false;
    }

    // Mobile status-tab QR
    const mobileQr = document.getElementById("mobile-qr-section");
    const mobileQrImg = document.getElementById("mobile-qr-image");
    const mobileLink = document.getElementById("mobile-ngrok-link");
    if (mobileQr && mobileQrImg && mobileLink) {
      mobileQrImg.src = "/qr.svg";
      mobileLink.href = data.ngrok_url;
      mobileLink.textContent = data.ngrok_url.replace("https://", "");
      mobileQr.hidden = false;
    }
  } catch { /* no tunnel active — silent */ }
}
...
function initMobileTabs() {
  ...
  tabBtns.forEach((btn) => {
    btn.addEventListener("click", () => {
      ...
      document.body.classList.remove("mobile-tab-chat", "mobile-tab-status");
      if (tab === "chat") document.body.classList.add("mobile-tab-chat");
      if (tab === "status") document.body.classList.add("mobile-tab-status");
    });
  });
}
