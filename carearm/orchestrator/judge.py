...
from carearm.orchestrator.prompts import SYSTEM_PROMPT
from carearm.types import SkillResult

if TYPE_CHECKING:
    from carearm.audit.log import AuditLog
    from carearm.integrations.calendar import CalendarClient
...
TOOLS: list[dict[str, Any]] = [
    {
        "name": "list_skills",
        "description": "List the robot's available skills with natural-language descriptions.",
    ...
    },
    {
        "name": "run_skill",
        "description": (
            "Run ONE rollout of a skill (e.g. feed_person, dispense_pills). Blocks until the "
            "rollout finishes. Returns success, confidence, and a note from camera verification."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
